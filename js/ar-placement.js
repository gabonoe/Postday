...
// Loads rey.glb (~1.7m tall), shows an animated star reticle on the floor,
...
export const initArPlacementScene = ({ onPlaced, onHintChange } = {}) => {
  ...
  const loadModel = async () => {
    ...
    const gltf = await loader.loadAsync('assets/3D/rey.glb');
    ...
    model.scale.setScalar((MODEL_HEIGHT_M / modelSize) * 1.5);
    ...
    reyModel = model;
    scene.add(reyModel);
    console.log('rey.glb loaded ✓  height =', MODEL_HEIGHT_M, 'm');
  ...
  const createBouncingBalls = (balonMesh) => {
    ...
    for (let i = 0; i < numBalls; i++) {
      ...
      ball.userData = {
        baseY: 0,
        bounceHeight: 0.6 + Math.random() * 0.3, // Increased from 0.3-0.5 to 0.6-0.9
  ...
  const createReticle = () => {
    ...
    const innerRing = new THREE.Mesh(
      new THREE.RingGeometry(0.20, 0.28, 48),
      new THREE.MeshBasicMaterial({
  ...
  const placeModel = () => {
    ...
    reyModel.position.y = reticlePos.y - 0; // lower by 0.1m to touch floor
    ...
    console.log('rey.glb placed at', reticlePos);
  };
  ...
  const initXrScene = ({ scene: s, camera: c, renderer }) => {
    ...
    loadModel().catch(err => console.error('Error cargando rey.glb:', err));
    createReticle();
  };
